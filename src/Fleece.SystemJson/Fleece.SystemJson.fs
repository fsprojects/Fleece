...
module Main =
    ...
    let inline toJson (x: 'T) : SjEncoding = toEncoding<SjEncoding, 'T> x

    /// Attempts to decode the value from its json encoding representation, using its default codec.
    let inline ofJson (x: SjEncoding) : Result<'T, DecodeError> = ofEncoding x

    /// Get the json value representation of the value, using its default codec.
    let inline toJsonValue (x: 'T) : JsonValue = toEncoding<SjEncoding, 'T> x |> SjEncoding.Unwrap
